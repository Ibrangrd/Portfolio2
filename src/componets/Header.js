...
const Header = ({ mode, toggleMode }) => {
  ...
  return (
    <div className={`${dark && "dark"} bg-slate-200 delay-150 pl-0`}>
      <div className="w-full border-slate-500 dark:border-b-2 dark:bg-slate-800 dark:text-white flex justify-between px-2 py-4 mb-2 shadow-xl font-bold h-16">
        <Link to="/" className="md:ml-2 hidden md:block lg:block hover:scale-110 text-sm  md:text-base">
          Ibran.dev
        </Link>
        <ul className="flex">
          <Link to="/">
            <li className="md:mx-4 ml-5 text-sm  md:text-base hover:bg-black hover:text-white hover:scale-110 md:w-16 w-14 h-6 md:h-8 rounded-xl px-2">
              Home
            </li>
          </Link>
          <Link to="/projects">
          ...
          <li className="md:mx-4 px-1 md:px-2" onClick={toggleMode}>
